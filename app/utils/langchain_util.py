...
class LangchainUtil:
    ...
    async def get_evalate_chaining_model(cls):
        ...
        if cls.evalate_chain_clovaX is None:
            ...
            다음 정보를 바탕으로 "RAG 모델의 응답"이 검색된 "참고 문서"의 내용에 얼마나 정확히 기반했으며, 질문에 얼마나 정확히 맞는 응답을 했는지 평가하세요:
            ...
            1. 질문에 대한 **정확한 정보가 문서에 포함되어 있는 경우**, RAG 모델 응답이 해당 정보를 **정확히 반영하고 왜곡 없이 요약 또는 인용했는지** 평가하세요.
